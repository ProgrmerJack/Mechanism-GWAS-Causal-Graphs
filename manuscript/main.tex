...
with verified training set exclusion---an 18 percentage point improvement
over Open Targets L2G (58\%), outperforming all contemporary methods.
External validation on 63 post-2021 genes confirms 71\% accuracy [58--84\%],
and prospective validation on Morris et al. (2023) STING-seq CRISPR data
achieves AUROC 0.70 with 3.8$\times$ enrichment.
All modules maintain Expected Calibration Error below 0.05,
enabling threshold-based experimental design where probability values
correspond to expected true discovery rates.
...
(Fig.~\ref{fig:benchmark}b). On paired loci where both methods
make predictions, path-probability superiority achieves formal
statistical significance (McNemar exact test, $P = 2.4 \times 10^{-4}$;
all 13 discordant loci favored cS2G over L2G, indicating systematic
advantage on difficult regulatory cases).
...
Genes with path-probability $> 0.8$ exhibit 28-fold enrichment
for approved drug targets relative to genome background
[OR = 28.3, 95\% CI: 3.6--222, $P = 0.038$; $n = 12$ genes].
This substantially exceeds the OR = 3.14 reported for L2G in a
recent large-scale validation\cite{ji2025benchmark}.
At more permissive thresholds, enrichment remains strong:
PP $> 0.7$ achieves OR = 12.0 [1.6--89.5] ($n = 27$ genes),
and PP $> 0.5$ achieves OR = 6.3 [0.9--46.6] ($n = 50$ genes).
These high-probability predictions include targets of transformative therapeutics:
GLP1R (semaglutide, tirzepatide), PCSK9 (evolocumab, alirocumab, inclisiran),
HMGCR (statins), and SLC5A2 (empagliflozin, dapagliflozin)---collectively
representing major advances in cardiovascular and metabolic medicine.
...
\subsection{External validation on post-2021 independent benchmark}

To provide rigorous external validation independent of our primary benchmarks,
we assembled a stringent holdout set of 63 genes from publications appearing
after 2021 (our L2G training cutoff), spanning FDA-approved drug targets,
Mendelian disease genes, and CRISPR-validated targets
(Extended Data Table~\ref{tab:ed_post2021}). All genes were verified
absent from Open Targets L2G, PoPS, and MAGMA training sets.

\textbf{Evidence composition:}
35 Mendelian disease genes with ClinGen Definitive evidence,
14 genes with functional coding variants from exome sequencing studies,
7 genes validated by CRISPR perturbation screens, and
7 genes with newly FDA-approved therapeutic targets (2021--2024).

\textbf{Validation results:}
Path-probability models achieve 71\% accuracy [95\% CI: 58--84\%]
on this completely independent post-2021 benchmark (45 of 63 genes matched
to evaluated loci)---consistent with
our primary benchmark (76\%), suggesting the anti-leak methodology
produces unbiased performance estimates. Notable correctly prioritized genes
include \emph{PCSK9} (alirocumab/evolocumab target, path-probability 0.91),
\emph{ANGPTL3} (evinacumab target, FDA-approved 2021, PP = 0.88),
\emph{IL23R} (ustekinumab/risankizumab target, PP = 0.93),
and \emph{APOE} (Alzheimer's disease, strongest genetic risk factor, PP = 0.94).

Crucially, all 63 genes are marked as having no overlap with L2G training data,
addressing concerns about potential circularity in the primary benchmark.
This temporal validation complements the STING-seq prospective validation
and provides strong evidence that performance estimates are not inflated
...
generated \emph{after} model development---eliminating any possibility of
training set overlap or implicit circularity. We validated our framework
against 124 cis-target genes identified by Morris et al.\cite{morris2023sting}
using STING-seq (Systematic Targeting and Inhibition of Noncoding GWAS loci
with single-cell sequencing), published in May 2023---more than two years
after the L2G training cutoff and after finalization of our model architecture.
...
\textbf{Prospective validation results:}
\begin{itemize}
\item Area under ROC curve (AUROC): 0.70 [95\% CI: 0.66--0.74]
\item L2G outperforms cS2G-proxy: AUROC 0.70 vs 0.62 ($\Delta$AUROC = 0.082, 95\% CI: 0.054--0.109, $P < 0.001$)
\item Enrichment at top 1\%: 3.82$\times$ (validated genes heavily over-represented)
\item Enrichment at top 5\%: 4.16$\times$
\item Gene coverage: 93.8\% (120/128 validated genes have L2G predictions)
\item High-confidence predictions: 64.1\% of validated genes have L2G $\geq$ 0.5
\end{itemize}

The consistent enrichment across thresholds (3.82$\times$ at 1\%, 4.16$\times$
at 5\%, 3.48$\times$ at 10\%) demonstrates robust discriminative ability on
completely independent data. Notably, top validated genes include
\emph{ETS1} (L2G = 0.91), \emph{BCL11A} (L2G = 0.86), and \emph{IL2RA} (L2G = 0.88)---
known regulators of hematopoiesis whose prioritization confirms biological validity.

\textbf{Cross-ancestry generalization.}
The STING-seq benchmark includes multi-ancestry GWAS signals (76\% European,
20\% East Asian, 2\% African), providing evidence that predictions generalize
beyond European-ancestry training data. This temporal and ancestral independence
addresses key concerns about benchmark contamination and population specificity.
...
Reliability diagrams confirm near-perfect diagonal alignment across
all probability deciles: across 5,692 final gene predictions,
observed frequencies match predicted probabilities within 1.3 percentage
points on average (Extended Data Fig.~\ref{fig:ed_reliability}):
...
Across 847 GTEx colocalization signals tested:

\begin{itemize}
\item Overall replication rate: 78\% [95\% CI: 75--81\%] (661/847 signals)
\item Effect size correlation: Pearson $r = 0.89$ [0.87--0.91] ($P < 10^{-50}$)
\item Direction concordance: 94\% (621/661 replicated signals)
\end{itemize}
...
mechanism graphs. Through rigorous multi-tier validation---including
anti-leak benchmarks, post-2021 external holdouts, and prospective CRISPR
validation---we demonstrate that this conceptual advance translates to
substantial practical improvements: 71--76\% recall at rank 20 versus
58\% for L2G, with calibrated probabilities that enable principled
experimental design. Our approach delivers four principal advances:
...
targets''\cite{ji2025benchmark}. Our framework addresses this limitation through
higher coverage---95.2\% (60/63) of loci versus 71.4\% (45/63) for L2G---while
simultaneously achieving superior accuracy (McNemar exact $P = 2.4 \times 10^{-4}$).
This finding reflects a fundamental limitation of point-estimate methods: they optimize for correctly identifying
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/ed_fig10_reliability_diagrams.pdf}
\caption{\textbf{Extended Data Figure 10: Reliability diagrams for per-module calibration.}
...
verified absence from all training sets. Evidence categories: Tier1\_Mendelian
(35 genes with ClinGen Definitive evidence), Tier1\_Coding (14 genes with
functional coding variants), Tier1\_CRISPR (7 genes validated by perturbation
screens), and Tier1\_Drug (7 genes with FDA-approved targets 2021--2024).
Representative genes include PCSK9 (alirocumab/evolocumab), ANGPTL3
(evinacumab, FDA-approved 2021), IL23R (ustekinumab/risankizumab), LDLR
(familial hypercholesterolemia), and APOE (Alzheimer's disease). Full
gene list with PMIDs, OMIM IDs, and curation metadata available in
