# Reproduction Bundle for Mechanism Graphs

This folder contains all reproduction data, scripts, and results needed to reproduce published claims.
...
**Evidence location:** `external_results/`
...
| `reproduction_metrics.json` | Metrics on external benchmarks |
...
- CRISPR-confirmed (Claussnitzer et al. 2015 NEJM)
...
All scripts in `scripts/` can be run to regenerate results:

```bash
# Reproduce large-scale calibration (UKBB E2G)
python scripts/reproduce_calibration_large_scale.py

# Reproduce external results (STING-seq, CRISPR)
python scripts/reproduce_external_data.py

# Reproduce all published claims
