#!/usr/bin/env python3
"""
Reproduce Published Claims Against Real Data

This script independently reproduces all quantitative claims from the published
paper against the actual data files, enabling full reproducibility.
...
Author: Reproduction Script for Mechanism Graphs Paper
"""
...
PAPER_DIR = BASE_DIR / "manuscript"
OUTPUT_DIR = BASE_DIR / "scripts" / "reproduction_output"
...
def reproduce_calibration_claims(cal_df: pd.DataFrame) -> Dict[str, Any]:
    """
    Reproduce published claim: ECE < 0.05 for all modules
    
    Published claims (Table 1):
...
def reproduce_benchmark_performance(cal_df: pd.DataFrame) -> Dict[str, Any]:
    """
    Reproduce published claim: Path-probability recall@20 = 76% vs L2G = 58%
...
def reproduce_replication_claims(repl_df: pd.DataFrame) -> Dict[str, Any]:
    """
    Reproduce published claims:
    - Overall replication rate: ≥78%
    - Effect size correlation: r ≥ 0.89
    - Direction concordance: ≥94%
    
    Note: Claims are MINIMUM thresholds. Exceeding them is valid.
    """
    results = {"claim": "Cross-study eQTL replication (minimum thresholds)", "validations": []}
    ...
    # Meets or EXCEEDS claim is valid
    meets_or_exceeds = replication_rate >= 0.78
    results["validations"].append({
        "metric": "replication_rate",
        "calculated": replication_rate,
        "manuscript_claim": "≥78%",
        "exceeds_claim": meets_or_exceeds,
        "note": f"Actual {replication_rate*100:.1f}% exceeds claimed 78%" if meets_or_exceeds else "Below threshold"
    })
    ...
    if len(valid_pairs) > 10:
        ...
        meets_or_exceeds_r = r >= 0.89
        results["validations"].append({
            "metric": "effect_size_correlation",
            "calculated": r,
            "manuscript_claim": "r ≥ 0.89",
            "exceeds_claim": meets_or_exceeds_r,
            "p_value": p,
            "note": f"Actual r={r:.3f} exceeds claimed r=0.89" if meets_or_exceeds_r else "Below threshold"
        })
    ...
    meets_or_exceeds_dir = direction_rate >= 0.94
    results["validations"].append({
        "metric": "direction_concordance",
        "calculated": direction_rate,
        "manuscript_claim": "≥94%",
        "exceeds_claim": meets_or_exceeds_dir,
        "note": f"Actual {direction_rate*100:.1f}% exceeds claimed 94%" if meets_or_exceeds_dir else "Below threshold"
    })
    
    results["valid"] = all(v.get("exceeds_claim", False) for v in results["validations"])
    return results

def reproduce_crispr_benchmark(cal_df: pd.DataFrame) -> Dict[str, Any]:
    """
    Reproduce published claim: CRISPR benchmark AUPRC = 0.71
    """
...
def reproduce_locus_statistics(locus_df: pd.DataFrame) -> Dict[str, Any]:
    """
    Reproduce locus-level statistics and mechanism paths.
    """
...
def reproduce_mechanism_graph_structure(graphs: Dict[str, Any]) -> Dict[str, Any]:
    """
    Check that mechanism graphs have correct structure.
    """
...
def reproduce_drug_targets(tier2_df: pd.DataFrame) -> Dict[str, Any]:
    """
    Reproduce drug target benchmark data.
    """
...
def generate_reproduction_report(all_results: Dict[str, Any]) -> str:
    """Generate comprehensive reproduction report."""
    report = []
    report.append("=" * 80)
    report.append("REPRODUCTION CLAIM REPORT")
    ...
    for section, results in all_results.items():
        ...
        if "valid" in results:
            status = "[PASS] REPRODUCED" if results["valid"] else "[WARN] NEEDS REVIEW"
    ...
    report.append("\n" + "=" * 80)
    report.append(f"OVERALL STATUS: {'[PASS] ALL CLAIMS REPRODUCED' if overall_valid else '[WARN] SOME CLAIMS NEED REVIEW'}")
    report.append("=" * 80)
...
def main():
    ...
    all_results = {
        "calibration": reproduce_calibration_claims(cal_df),
        "benchmark_performance": reproduce_benchmark_performance(cal_df),
        "replication": reproduce_replication_claims(repl_df),
        "crispr_benchmark": reproduce_crispr_benchmark(cal_df),
        "locus_statistics": reproduce_locus_statistics(locus_df),
        "mechanism_graphs": reproduce_mechanism_graph_structure(graphs),
        "drug_targets": reproduce_drug_targets(tier2_df)
    }
    
    # Generate report
    report = generate_reproduction_report(all_results)
    print(report)
    
    # Save results
    output_file = OUTPUT_DIR / "reproduction_report.txt"
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(report)
    
    # Save JSON results
    json_file = OUTPUT_DIR / "reproduction_results.json"
